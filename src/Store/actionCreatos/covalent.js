...
export const fetchPortfolio = (chainId, address) => {
  return async (dispatch) => {
    ...
    return await axios
      .get(
        `${COVALENT_URL}/${chainId}/address/${address}/portfolio_v2/?quote-currency=USD&format=JSON&key=${process.env.REACT_APP_COVALENT_API_KEY}`
      )
      .then((response) => {
...
export const fetchNFTDetails = (chainId) => {
  return async (dispatch) => {
    ...
    return await axios
      .get(
        `https://api.covalenthq.com/v1/${chainId}/nft_market/?key=${process.env.REACT_APP_COVALENT_API_KEY}&page-size=10`
      )
      .then((response) => {
        if (response.status === 200) {
          dispatch({
