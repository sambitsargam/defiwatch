...
function Appy() {
  ...
  useEffect(() => { // Only run once when the component mounts
    ...
    const fetchTransactions = async () => {
      const add = address;
      console.log(address);
      const response = await fetch(`https://api-testnet.polygonscan.com/api?module=account&action=txlist&address=${add}&apikey=ZBCNZDY8EQM37IC6P1EKWUCR6265ZF9ZJC`);
        ...
        <Tbody>
          {transactions.map((tx) => (
            <Tr key={tx.hash}>
              ...
              <Td>{ConvertValue(tx.value)} MATIC</Td>
            </Tr>
          ))}
        </Tbody>
      </Table>
